...
class Leaderboard:
    ...
    Daily leaderboard expires after 180 seconds.
    """
    ...
    def __init__(
        ...
        if scope == "daily":
            ...
            self.r.expire(self.key, 180)  # 180 seconds
        elif scope == "alltime":
    ...
    def get_players_in_score_range(self, min_score: float, max_score: float) -> List[Dict]:
        """
        Get all players within a score range (inclusive).
        Uses ZREVRANGEBYSCORE to return highest scores first.
        """
    ...
    def get_ttl(self) -> Optional[int]:
        """
        Get remaining TTL in seconds for the leaderboard key.
        Returns None if the key has no expiry (alltime).
...
def demo():
    ...
    print("\nPlayers with score between 1800 and 2200 (daily):")
    for e in daily_lb.get_players_in_score_range(1800, 2200):
        print(f" #{e['rank']} {e['player']}: {e['score']}")

    print(f"\nDaily TTL: {daily_lb.get_ttl()} seconds remaining")
    ...
    print("\nPlayers with score between 1800 and 2200 (all-time):")
    for e in alltime_lb.get_players_in_score_range(1800, 2200):
        print(f" #{e['rank']} {e['player']}: {e['score']}")

    print(f"\nAll-time TTL: {alltime_lb.get_ttl()} (None = never expires)")
